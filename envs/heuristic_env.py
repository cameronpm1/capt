...
class heuristicEnv(satGymEnv):
    ...
    def action_space(
        ...
        space['evader'] = spaces.Box(low=-1, high=1, shape=(self.action_dim,), dtype=np.float32)
    ...
    def observation_space(
        ...
        #        space[label] = spaces.Box(low=-np.inf, high=np.inf, shape=(len(obs[label]),), dtype=np.float32)
        space['evader'] = spaces.Box(low=-np.inf, high=np.inf, shape=(len(obs['evader']),), dtype=np.float32)
    ...
    def step(self,action):
        ...
        obs = {'evader': obs['evader']}
        rew = {'evader': rew['evader']}
        terminated = {'evader': terminated['evader'], '__all__': terminated['__all__']}
        truncated = {'evader': truncated['evader'], '__all__': truncated['__all__']}

        print(self._step)
        return obs, rew, terminated, truncated, {'goal_count':self.goal_count}
